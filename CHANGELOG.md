...
- Added a `TranslationProvider` protocol (`core/language/translate.py`) with a no-op default and an OpenAI/Azure-backed server-side fallback, used only when the client supplies no translation, cached on `(model, text, target_language)`.
...
- No corpus re-index or re-embed needed. The embedding model, `EMBEDDING_MODEL`, and `EMBEDDING_DIMENSIONS` are unchanged; this release's cross-language support works via query translation and rank fusion at query time, not a change to stored embeddings.
