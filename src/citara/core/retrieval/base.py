...
def apply_source_filters(
    ...
    *,
    ...
    """Apply the shared retrieval filters to a statement.

    Returns `(statement, matchable)`. `matchable` is False when a filter can
    provably match nothing (e.g. unknown entity slugs), so callers return an
    empty list without running the query.

    Every backend routes through this so a filter fix lands everywhere at
    once -- keyword, vector, and FTS previously carried three near-identical
    copies of this logic.
    """

    statement = statement.where(Chunk.tenant_id == tenant_id, Source.tenant_id == tenant_id)
...
def result_from(source: Source, chunk: Chunk, score: float) -> SearchResult:
    """Build a SearchResult from an ORM row pair. Shared by every backend."""
    metadata = source.metadata_json or {}
    return SearchResult(
        chunk_id=chunk.id,
        source_id=source.id,
        ...
        end_ms=chunk.end_ms,
        transcript_provenance=metadata.get("transcript_provenance"),
        timestamp_provenance=metadata.get("timestamp_provenance"),
        timestamp_precision=metadata.get("timestamp_precision"),
