...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Hermes Knowledge Vault maintenance helpers")
    sub = parser.add_subparsers(dest="command", required=True)
    reset = sub.add_parser("reset", help="Safely remove selected HKB derived/artifact state")
...
def print_actions(actions: list[tuple[str, Path | str]], *, dry_run: bool) -> None:
    print("DRY RUN: planned HKB reset actions" if dry_run else "Executing HKB reset actions")
